...
response = WS.sendRequest(findTestObject('Details/Fetch Banks'))
...
WS.verifyElementPropertyValue(response, 'message', 'The process was completed successully')

//Verify that the response has the value for bank code 100001 in the body
WS.verifyElementPropertyValue(response, 'data.100001.bank_code', '100001')

//Verify that the response has the value for bank code 100002 in the body
WS.verifyElementPropertyValue(response, 'data.100002.bank_code', '100002')

//Verify that the response has the value for bank code 100003 in the body
WS.verifyElementPropertyValue(response, 'data.100003.bank_code', '100003')

//Verify that the response has the value for the name of the wallet in the body
WS.verifyElementPropertyValue(response, 'data.100001.name', 'FETS (My Wallet)')
